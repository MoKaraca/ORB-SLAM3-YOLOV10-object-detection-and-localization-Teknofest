...
class MonoDriver(Node):
    ...
    def get_image_dataset_asl(self, exp_dir, agent_name = "mav0"):
        ...
        raw_file_list = os.listdir(imgz_file_dir)
        img_extensions = ('.png', '.jpg', '.jpeg')
        filtered_files = [f for f in raw_file_list if f.lower().endswith(img_extensions)]
        
        imgz_file_list = natsort.natsorted(filtered_files, reverse=False)
    ...
    def run_py_node(self, idx, imgz_name):
        ...
        img_name_prefix = imgz_name.split(".")[0]
        try:
            # Attempt to read the filename as a Unix timestamp in nanoseconds (EuRoC format)
            timestamp_ns = int(img_name_prefix)
        except ValueError:
            # If the filename isn't a timestamp (e.g. "frame_000000"), generate a fake one
            # We assume 30 FPS, which is ~33,333,333 nanoseconds per frame
            timestamp_ns = self.frame_id * 33333333
        ...
        # Add timestamp natively into the standard ROS 2 image header
        img_msg.header.stamp.sec = timestamp_ns // 1000000000
        img_msg.header.stamp.nanosec = timestamp_ns % 1000000000
